...
if __name__ == '__main__':
  ...
  dataText_features = dataText_vectorizer.fit_transform(documents)
  ...
  for day in range(len(dirsList)):
    ...
    if  day > 5 and day % 5 == 0:
      ...
      header1.extend(["@data\n"])
      numpy.savetxt('../atrocitiesdata/tfidfkmeans/ch_'+str(chunk)+'.arff', data, fmt='%f,'*(data.shape[1]-1)+'%i', delimiter=',',header="\n".join(header1), comments='')
      chunk+=1
      Trainingindexes=[]
      continue
      ...
      '''
      fname='../atrocitiesdata/tfidfkmeans/ch_'+str(chunk)+"_lbl.csv"
