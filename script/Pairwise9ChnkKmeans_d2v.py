...
dataText_vectorizer = TfidfVectorizer(analyzer = "word",tokenizer = tokenize, preprocessor = None, stop_words = 'english', max_features = 10000, ngram_range=(1, 3))
...
for day in range(nTrainingdays,len(dirsList)):
    ...
    if day % 5 == 0: #if len(dayindexes)>1:
        ...
        pred_y=[]
        for x1 in range(kmeans.labels_.shape[0]):
          for x2 in range(x1+1,kmeans.labels_.shape[0]):
              if kmeans.labels_[x1]==kmeans.labels_[x2]:
                  pred_y.append(1)
              else:
                  pred_y.append(-1)
...
with codecs.open(logfilename,'a',"utf-8") as f:
    f.write("d2vKmeans,nTrainingdays,TPR,TNR,FPR,FNR,precPos,recallPos,precNeg,recallNeg,f1,f2,acc,\n" + "\t".join(log))
