...
if __name__ == '__main__':
  ...
  for day in range(len(dirsList)):
    ...
    if day > 5 and day % 5 == 0:
      ...
      header1.extend(["@data\n"])
      numpy.savetxt('../atrocitiesdata/doc2vecbatches/ch_'+str(chunk)+'.arff', data, fmt='%f,'*(data.shape[1]-1)+'%i', delimiter=',',header="\n".join(header1), comments='')
      chunk+=1
      Trainingindexes=[]
      #continue
      kmeans = KMeans(n_clusters=len(GTclusterslabels[0]), random_state=0).fit(datax)
      '''
      fname='../atrocitiesdata/doc2vecbatches/ch_'+str(chunk)+"_lbl.csv"
